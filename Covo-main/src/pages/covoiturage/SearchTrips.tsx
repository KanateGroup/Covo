  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#232526] to-[#414345] py-10 px-4 lg:px-24">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-extrabold text-white mb-2">Trajets disponibles</h1>
        ...
        <Card className="mb-8 bg-white/10 border-white/20 shadow-lg">
          <CardContent className="py-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <div className="text-2xl font-bold text-[#b6ffb0]">{trips.length}</div>
              <div className="text-white/80">trajets disponibles</div>
            </div>
            <div className="flex flex-wrap gap-2 items-center">
              <span className="text-white/70 font-semibold">Destinations :</span>
              {[...new Set(trips.map(t => t.destination))].map((dest, i) => (
                <Badge key={i} className="bg-[#4ecdc4] text-black">{dest}</Badge>
              ))}
            </div>
            <div className="text-white/60 text-sm text-right">
              Dernier ajout : {trips.length > 0 ? formatDate(trips[0].date) : '—'}
            </div>
          </CardContent>
        </Card>
        <AnimatePresence>
